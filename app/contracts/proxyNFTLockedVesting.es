{{
    ...
    val NFTLockedVestingContract = {NFTLockedVestingContract}
    val ErgUSDOracleNFT = fromBase64("{ErgUSDOracleNFT}")
    val SigUSDTokenId = fromBase64("{SigUSDTokenId}")
    ...
    val nergPerUSD = if (CONTEXT.dataInputs.size == 0) 1 else CONTEXT.dataInputs(0).R4[Long].get
    ...
    val sellerSigUSD = if (OUTPUTS(3).tokens.size == 2) if (OUTPUTS(3).tokens(1)._1 == SigUSDTokenId) OUTPUTS(3).tokens(1)._2 else 0 else 0
    val ergValueInSigUSD = OUTPUTS(3).value * 100 / nergPerUSD
    val enoughSigUSD = (sellerSigUD + ergValueInSigUSD) >= requiredSigUSD
