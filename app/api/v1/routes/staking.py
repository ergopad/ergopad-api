...
from paideia_contracts.contracts.staking import CreateAddStakeProxyTransaction, CreateStakeProxyTransaction, CreateUnstakeProxyTransaction, NETAConfig, PaideiaConfig, PaideiaTestConfig, EGIOConfig, AHTConfig
...
def unstake(req: UnstakeRequest, project: str = "ergopad"):
    try:
        ...
        for ass in stakeBox["assets"]:
            # ergopad only
            if ass['tokenId'] == 'd71693c49a84fbbecd4908c94813b46514b18b67a99952dc1e6e4791556de413':
                try: remaining = int(ass['amount'])
                except: remaining = 0
        
        # logging.debug('unstake::find stakeBox')        
        currentTime = int(time()*1000)
        amountToUnstake = min(int(req.amount*10**stakedTokenInfo["decimals"]), remaining)
        ...
        for ass in stakeStateBox["assets"]:
            if ass['tokenId'] == '1028de73d018f0c9a374b71555c5b8f1390994f2f41633e7b9d68f77735782ee':
                try: stakeStateAmountToUnstake = int(ass['amount'])
                except: stakeStateAmountToUnstake = 0

        if not partial: stakeStateAmountToUnstake += 1
        outputs = []
        ...
        if partial:
            outputs.append(appKit.buildOutBox(
                ...
                tokens = {
                    sc["stakeTokenID"]: 1,
                    sc["stakedTokenID"]: remaining - amountToUnstake # stakeBox["assets"][1]["amount"]-amountToUnstake
                },
                registers=[
...
def stakingStatus(project: str):
    try:
        ...
        dailyEmission = cycleEmission*86400000/ErgoValue.fromHex(res['str4']).getValue().apply(4)
...
def stakedv2(project: str, req: AddressList):
    try:
        ...
        if VERBOSE: logging.debug('stakedv2.sql')
        sql = text(f'''
            select 
                  s.address
                , s.project_name
        ...
        for row in res:
            if VERBOSE: logging.debug(f'''row {row['address']}''')
            # properly convert amount
            amt = Decimal(0)
            try: amt = Decimal(row['amount'])
            except: pass
            totalStaked += amt
            
            # init stakePerAddress
            if row['address'] not in stakePerAddress:
                if VERBOSE: logging.debug('stakedv2.init2')
                stakePerAddress[row['address']] = {'totalStaked': Decimal(0), 'stakeBoxes': []}
                tokenName = row['token_name']
            
            # penalty
            if VERBOSE: logging.debug('stakedv2.penalty')
            ...
            if project in ['ergopad', 'egio']:
                try:
                    ...
                    if VERBOSE: logging.debug(f'penalty: {penalty}')
                    penaltyPct = validPenalty(penalty)
                    if VERBOSE: logging.debug(f'penalty: {penaltyPct}')
                    penaltyEndTime = int(penalty + 8*week)
                    if VERBOSE: logging.debug(f'penalty: {penaltyEndTime}')
                except Exception as e:
                    if VERBOSE: logging.debug(f'invalid penalty {e}')
                    pass
            
            # summarize
            if VERBOSE: logging.debug('stakedv2.summary')
        ...
        if VERBOSE: logging.debug('stakedv2.return')
        return {
            'project': project,
            'tokenName': tokenName,
...
def allStaked(req: AddressList):
    try:
        ...
        if VERBOSE: logging.debug('allStaked.sql')
        sql = text(f'''
            select 
                  s.address
                , s.project_name
        ...
        for row in res:
            ...
            if row['address'] not in stakePerAddress[project]:
                if VERBOSE: logging.debug('allStaked.init2')
                stakePerAddress[project][row['address']] = {'totalStaked': Decimal(0), 'stakeBoxes': []}
                if VERBOSE: logging.debug('allStaked.init3')
